...
| =--pg-mtd-info                 | print Postgres Metadata Info to cerr                           |
...
following are options for controlling sql generation. This allows controlling the process of
sql generation.

| =--types-to-include=           | file that white lists data types                               |
| =--tables-to-include=          | file that white list tables by name                            |
| =--routines-to-include=        | file that white list routines/functions by name                |
| =--routines-to-include=        | file that white list routines/functions by name                |
| =--aggregates-to-include=      | file that white list aggregates to include by name             |
| =--operators-to-include=       | file that white list operators to include                      |
| =--max-joined-tables=          | maximum number of joined tables in the from clause default 100 |
| =--max-value-expr-depth=       | maximum depth of the value expr tree default 50                |
| =--avoid-correlated-references | avoid generating correlated references                         |
| =--stmt-types-to-include=      | file that white list statement types                           |
| =--table-ref-types-to-include= | file that white list table ref types                           |
| =--value-expr-types-to-include=| file that white list value expr types                          |
